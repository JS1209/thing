...
int open_wheel() {
    if (check_device_available(RULE_VEN_ID, RULE_PROD_ID)) {
        handle = hid_open(RULE_VEN_ID, RULE_PROD_ID, NULL);
        if (!handle) {
...
int setup_wheelbase() {
    if (hid_init()) {
        ...
        log_perror_to_file("/fanatec/fanatec_functions.setup_wheelbase", "Error: Wheel could not be opened, hid_init failed");
        return 1;
    }
    if (open_wheel()) {
